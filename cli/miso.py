...
- Download: Uses MISO bundle (with Conan dependency resolution) to download package + dependencies
"""
...
def get_conan_executable():
    ...
    # Try venv first
    script_dir = Path(__file__).parent.parent  # Go up to MISO root (conancrates.py is in conancrates/)
...
def check_package_exists(server_url, package_ref, package_id):
    """
    Check if a specific binary package already exists on MISO server.

    Args:
        server_url: MISO server URL
        package_ref: Package reference like "boost/1.81.0"
...
def upload_package(server_url, recipe_path, binary_path, package_ref, package_id=None, dependency_graph=None, rust_crate_path=None, readme_path=None):
    """
    Upload package to MISO server.

    Args:
        server_url: MISO server URL (e.g., http://localhost:7007)
    ...
    """
    upload_url = f"{server_url}/api/conancrates/package/upload"
...
def cmd_upload(args):
    ...
    if non_release_packages:
        ...
        print(f"  MISO only accepts release versions (no -rc, -beta, -alpha, etc.)")
...
def cmd_download_rust_crates(args):
    ...
    query_url = f"{server_url}/api/conancrates/packages/{package_name}/versions/{version}/rust-crate"
    try:
    ...
    info_url = f"{server_url}/api/conancrates/packages/{package_name}/versions/{version}/binaries/{package_id}/info"
    ...
    crate_url = f"{server_url}/api/conancrates/packages/{package_name}/versions/{version}/binaries/{package_id}/rust-crate"
    ...
    if dependencies:
        ...
        for dep in dependencies:
            ...
            dep_url = f"{server_url}/api/conancrates/packages/{dep_name}/versions/{dep_version}/binaries/{dep_package_id}/rust-crate"
...
def cmd_download(args):
    """
    Download package and dependencies from MISO and restore to local Conan cache.
    ...
    bundle_url = f"{server_url}/api/conancrates/packages/{package_name}/versions/{version}/bundle?os={os_name}&arch={arch}&compiler={compiler}&compiler_version={compiler_version}&build_type={build_type}"
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            try:
                binaries_url = f"{server_url}/api/conancrates/packages/{package_name}/versions/{version}/binaries"
...
def cmd_generate_rust_crate(args):
    ...
    print(f"MISO Rust Crate Generator")
    print(f"{'='*60}")
    print(f"Package: {package_ref}")
    print(f"Profile: {profile}")
...
def main():
    parser = argparse.ArgumentParser(
        description='MISO CLI - Upload and download Conan packages to/from MISO registry'
    )
    parser.add_argument(
        '--server',
        default='http://localhost:7007',
        help='MISO server URL (default: http://localhost:7007)'
    )
